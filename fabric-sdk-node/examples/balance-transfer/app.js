...
app.get('/getChaincode', function(req, res) {
    res.json({
        success: true,
        message: JSON.stringify(chaincodeList)
    });
});
...
app.get('/addchannel', function(req, res) {
    ...
    if (channelList[username] != null){
        result = channelList[username];
    }
    res.json({
        success: true,
        //message: JSON.stringify(result)
        message: JSON.stringify(channelList)
    });
});

//get orgs
app.get('/allOrgs', function(req, res) {
	...
	jwt.verify(token, app.get('secret'), function(err, decoded) {
		if (err) {
			...
			.then(function(message) {
				if (channelList[decoded.username] == null){
                    channelList[decoded.username] = [];
                }
                channelList[decoded.username].push(channelName);
                res.send(message);
			});
		}
	});
...
app.post('/chaincodes', function(req, res) {
	...
	jwt.verify(token, app.get('secret'), function(err, decoded) {
		if (err) {
			...
			.then(function(message) {
				for (var i in peers){
				  if (!chaincodeList[peers[i]]){
				    chaincodeList[peers[i]] = {};
				  }
				  var ccData = {
				  ...
				  chaincodeList[peers[i]][chaincodeName] = ccData;
				}
				res.send(message);
			});
		}
