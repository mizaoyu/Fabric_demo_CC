...
export class QueryTrans extends Component {
  constructor(props) {
      super(props);
      this.state = {
        queryResult:'',
  ...
  componentDidMount() {
    ...
    fetch('http://localhost:4000/tIds', {
        method: 'GET',
        headers: {
          'cache-control': 'no-cache',
          'content-type': 'application/json',
        ...
        if (!data.success){
            ...
            console.log(data);
            this.setState({ tIds: JSON.parse(data.message) });
        }
      }).catch(function(error) {
        console.log('request failed', error);
        alert(error);
  ...
  handleSubmit(event) {
      ...
      var url = 'http://localhost:4000/channels/mychannel/transactions/'
               + this.state.checkedTID +
               '?peer=' + peer;
      ...
      fetch(url, {
        ...
        alert("Query Transaction successfully!");
      }).catch(function(error) {
        console.log('request failed', error);
        alert(error);
      });
  ...
  render() {
    ...
    var transactionData = this.state.tIds.map((tId) => (
            <div className="ui-radio on" data-radio="group">
                <input className="input-hidden" type="radio" name="tId" value={tId}></input>
                <span className="radio-text">{tId}</span>
            </div>
    ));
    ...
    <div>
      ...
      <form className="CCh-form" onSubmit={this.handleSubmit}>
          <fieldset>
            <legend>Choose the existing channel to query transaction on</legend>
            <div onChange={this.handleRadioChange}>
              {channelData}
            </div>
          </fieldset>
          <fieldset>
            <legend>Choose the peer to query transaction on</legend>
            <div onChange={this.handleRadioChange}>
              {peerData}
            </div>
          </fieldset>
