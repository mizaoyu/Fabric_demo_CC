...
chaincodeQuery () {
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME -n marbles -c $ARGS >&log.txt
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		...
		peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n marbles -c $ARGS >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeQuery 0 '{"Args":["readMarble","marble2"]}'

chaincodeQuery 0 '{"Args":["getHistoryForMarble","marble1"]}'

chaincodeQuery 0 '{"Args":["queryMarblesByOwner","jerry"]}'

installChaincode 2

sleep 1s

chaincodeQuery 2 '{"Args":["queryMarblesByOwner","jerry"]}'
